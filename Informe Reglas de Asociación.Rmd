...
## Obtención de Ítems frecuentes por algoritmo A priori
...
Se han encontrado un total de 1323 itemsets frecuentes que superan el soporte mínimo de 0.002, la mayoría de ellos (779) formados por un item. En el siguiente listado se muestran los 6 itemsets con mayor soporte que, como cabe esperar, son los formados por items individuales (los itemsets de menor tamaño).

## Obtención de Ítems frecuentes por algoritmo Eclat
...
itemsets_filtrado
```
Se observa que se han encontrado 27 itmensets que contienen el item Organic Avocado y se muestra una lista de los 5 primeros:
...
Es  decir en la base de datos existen 2499 subconjutnos de otros itemsets.
...
#### Eclat
```{r echo=FALSE}
reglas_filtradas2 <- subset(reglas2,
                           subset = lhs %ain% "Blueberry Whole Milk Yogurt Pouch" &
                                    confidence > 0.9)

inspect(reglas_filtradas2)
```
...
#### Eclat
```{r echo=FALSE}
filtrado_reglas2 <- subset(x = reglas2,
                          subset = lhs %ain% c("Blueberry Whole Milk Yogurt Pouch","Organic Whole Milk Strawberry Beet Berry Yogurt Pouch"))
inspect(filtrado_reglas2)
```
...
#### Eclat
```{r echo=FALSE}
reglas_maximales2 <- reglas[is.maximal(reglas2)]
reglas_maximales2

inspect(reglas_maximales2)
```
...
#### Eclat
```{r echo=FALSE}
testFisher2 <- interestMeasure(reglas2, 
                              measure = "fishersExactTest",
                              transactions = transacciones)
...
```{r echo=FALSE}
metricas2 <- interestMeasure(reglas2, measure = c("coverage", "fishersExactTest"),
                            transactions = transacciones)
metricas2
